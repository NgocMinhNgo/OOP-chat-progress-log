...
public class MainAppController implements Initializable {
    ...
    private ConversationSidebarController sidebarController;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        try {
            // Load sidebar component
            FXMLLoader sidebarLoader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/ConversationSidebar.fxml"));
            sidebarContainer.getChildren().add(sidebarLoader.load());
            sidebarController = sidebarLoader.getController();
            sidebarController.setup(conversations, this::handleConversationSelection);

            //newConversationButton.setOnAction((event) -> showProductSelectionView());
...
//        showWelcomeView();
        showFallbackWelcomeView();

    }

    private void showWelcomeView() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/WelcomeView.fxml"));
...
//            contentArea.getChildren().setAll((Node) loader.load());
//
//            // Lấy controller của WelcomeView và thiết lập callback
//            WelcomeViewController welcomeController = loader.getController();
//            welcomeController.setOnStartNewConversation(this::showProductSelectionView);
//
//        } catch (IOException e) {
//            showFallbackWelcomeView();
//        }
//    }
    ...
    private void showFallbackWelcomeView() {
        ...
        descLabel.getStyleClass().add("welcome-description");
        ...
        startButton.setOnAction(e -> {
            System.out.println("Nút đã được click!");  // Kiểm tra xem có in ra không?
...
//        try {
//            FXMLLoader loader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/ProductAnalysisSelectionView.fxml"));
//            Pane selectionView = loader.load();
//            ProductAnalysisSelectionController controller = loader.getController();
//            controller.setOnSelectionConfirmed(pair ->
...
private void showProductSelectionView() {
    try {
        System.out.println("Đang load FXML...");
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/ProductAnalysisSelectionView.fxml"));
        Pane selectionView = loader.load();
        System.out.println("Load FXML thành công!");

        ProductAnalysisSelectionController controller = loader.getController();
        controller.setOnSelectionConfirmed(pair -> createNewConversation(pair.getKey(), pair.getValue()));

        contentArea.getChildren().setAll(selectionView);
    } catch (IOException e) {
        System.err.println("Lỗi khi load FXML:");
        e.printStackTrace();
        showFallbackWelcomeView();  // Fallback nếu lỗi
    }
}
    ...
    private void initializeNewConversation(Conversation conversation) {
        try {
            ...
            //contentArea.getChildren().setAll(loader.load());
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/LoadingPane.fxml"));
            Node loadingPane = loader.load(); // rõ ràng đây là 1 Node
            contentArea.getChildren().setAll(loadingPane); // Java hiểu đúng

            new Thread(() -> {
                try {
                    System.out.println("Bắt đầu tạo phones");
                    ArrayList<Phone> products = createSamplePhones();
                    System.out.println("Hoàn thành tạo phones");

                    Platform.runLater(() -> {
                        try {
                            System.out.println("[UI Thread] Đang cập nhật giao diện...");
                            FXMLLoader chatLoader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/ChatArea.fxml"));
                            Pane chatArea = chatLoader.load();
                            System.out.println("Load FXML thành công!");
                            ChatAreaController chatController = chatLoader.getController();
                            chatController.initializeWithData(conversation, products);
    ...
    private void loadExistingConversation(Conversation conversation) {
        ...
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/TryUIIdea01/application/ChatArea.fxml"));
            Pane chatArea = loader.load();
            System.out.println("Load FXML thành công!");
            ChatAreaController controller = loader.getController();
            controller.initializeWithData(conversation, createSamplePhones());
            contentArea.getChildren().setAll(chatArea);
        } catch (IOException e) {
            e.printStackTrace();
        }
